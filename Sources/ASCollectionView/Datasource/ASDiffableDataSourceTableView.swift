...
@available(iOS 13.0, *)
class ASDKDiffableDataSourceTableView<SectionID: Hashable>: ASDiffableDataSource<SectionID>, ASTableDataSource
{
    /// The type of closure providing the cell.
    public typealias Snapshot = ASDiffableDataSourceSnapshot<SectionID>
    public typealias CellProvider = (ASTableNode, IndexPath, ASCollectionViewItemUniqueID) -> ASCellNode?

    private weak var tableView: ASTableNode?
    private let cellProvider: CellProvider
    private var indexTitles: [(Int, String)] = []
    ...
    public init(tableView: ASTableNode, cellProvider: @escaping CellProvider)
    {
        self.tableView = tableView
        self.cellProvider = cellProvider
        super.init()
    ...
    func updateCellSizes(animated: Bool = true)
    {
        guard let tableView = tableView, canRefreshSizes, !tableView.view.visibleCells.isEmpty else { return }
        CATransaction.begin()
        if !animated
        {
            CATransaction.setDisableActions(true)
    ...
    func tableNode(_ tableNode: ASTableNode, nodeBlockForRowAt indexPath: IndexPath) -> ASCellNodeBlock {
        ...
        let nodeBlock: ASCellNodeBlock = {
            self.cellProvider(tableNode, indexPath, itemIdentifier) ?? ASCellNode()
        }
        return nodeBlock
    }
