...
@available(iOS 13.0, *)
internal protocol ASDKSectionDataSourceProtocol
{
    var endIndex: Int { get }
    func getIndexPaths(withSectionIndex sectionIndex: Int) -> [IndexPath]
    func getItemID<SectionID: Hashable>(for index: Int, withSectionID sectionID: SectionID) -> ASCollectionViewItemUniqueID?
    func getUniqueItemIDs<SectionID: Hashable>(withSectionID sectionID: SectionID) -> [ASCollectionViewItemUniqueID]
    func content(forItemID itemID: ASCollectionViewItemUniqueID) -> ASCellNode
    func content(supplementaryID: ASSupplementaryCellID) -> AnyView?
    var supplementaryViews: [String: AnyView] { get set }
    func getTypeErasedData(for indexPath: IndexPath) -> Any?
    func onAppear(_ indexPath: IndexPath)
...
@available(iOS 13.0, *)
protocol ASDKDataSourceConfigurableCell
{
    func setContent<Content: View>(itemID: ASCollectionViewItemUniqueID, content: Content)
//    var disableSwiftUIDropInteraction: Bool { get set }
//    var disableSwiftUIDragInteraction: Bool { get set }
...
@available(iOS 13.0, *)
internal struct ASDKSectionDataSource<DataCollection: RandomAccessCollection, DataID, Content, Container>: ASDKSectionDataSourceProtocol where DataID: Hashable, Content: ASCellNode, Container: ASCellNode, DataCollection.Index == Int
{
    typealias Data = DataCollection.Element
    var data: DataCollection
    var dataIDKeyPath: KeyPath<Data, DataID>
...
internal struct ASSectionBindingDataSource<T: MutableCollection & RandomAccessCollection, DataID, Content, Container>: ASSectionDataSourceProtocol where DataID: Hashable, Content: View, Container: View, T.Index == Int, T.Element: Identifiable & ComputableHeight
{
    ...
    var data: DataCollection
    var dataIDKeyPath: KeyPath<T.Element, DataID>
    var container: (Content, ASCellContext) -> Container
    var content: (Binding<T.Element>, ASCellContext) -> Content
    ...
    var onCellEvent: OnCellEvent<T.Element>?
    var dragDropConfig: ASDragDropConfig<T.Element>
    var shouldAllowSwipeToDelete: ShouldAllowSwipeToDelete?
    var onSwipeToDelete: OnSwipeToDelete<T.Element>?
    var contextMenuProvider: ContextMenuProvider<T.Element>?
    var selfSizingConfig: (SelfSizingConfig)?
    ...
    var endIndex: Int { data.wrappedValue.endIndex }

    func getIndex(of itemID: ASCollectionViewItemUniqueID) -> Int?
    {
        data.wrappedValue.firstIndex(where: { $0[keyPath: dataIDKeyPath].hashValue == itemID.itemIDHash })
    }
    ...
    func cellContext(for index: Int) -> ASCellContext
    {
        ASCellContext(
            ...
            isSelected: isSelected(index: index),
            index: index,
            isFirstInSection: index == data.wrappedValue.startIndex,
            isLastInSection: index == data.wrappedValue.endIndex - 1)
    }
    ...
    {
        guard let itemIndex = getIndex(of: itemID) else { return nil }
        let item = Binding( get: { data.wrappedValue[itemIndex] }, set: { data.wrappedValue[itemIndex] = $0 })
        let context = cellContext(for: itemIndex)
        let view = content(item, context)
        return container(view, context)
    }
    ...
    func getTypeErasedData(for indexPath: IndexPath) -> Any?
    {
        data.wrappedValue[safe: indexPath.item]
    }

    func getIndexPaths(withSectionIndex sectionIndex: Int) -> [IndexPath]
    {
        data.wrappedValue.indices.map { IndexPath(item: $0, section: sectionIndex) }
    }

    func getItemID<SectionID: Hashable>(for index: Int, withSectionID sectionID: SectionID) -> ASCollectionViewItemUniqueID?
    {
        data.wrappedValue[safe: index].map { getItemID(for: $0, withSectionID: sectionID) }
    }

    func getItemID<SectionID: Hashable>(for item: T.Element, withSectionID sectionID: SectionID) -> ASCollectionViewItemUniqueID
    {
        ASCollectionViewItemUniqueID(sectionID: sectionID, itemID: item[keyPath: dataIDKeyPath])
    }
    ...
    func onAppear(_ indexPath: IndexPath)
    {
        guard let item = data.wrappedValue[safe: indexPath.item] else { return }
        onCellEvent?(.onAppear(item: item))
    }
    ...
    func onDelete(indexPath: IndexPath) -> Bool
    {
        guard let item = data.wrappedValue[safe: indexPath.item], let onDelete = onSwipeToDelete else { return false }
        let didDelete = onDelete(indexPath.item, item)
        return didDelete
    }
    ...
    func getDragItem(for indexPath: IndexPath) -> UIDragItem?
    {
        ...
        guard let item = data.wrappedValue[safe: indexPath.item] else { return nil }
    ...
    func getContextMenu(for indexPath: IndexPath) -> UIContextMenuConfiguration?
    {
        guard
            let menuProvider = contextMenuProvider,
            let item = data.wrappedValue[safe: indexPath.item]
        else { return nil }
    ...
    func shouldHighlight(_ indexPath: IndexPath) -> Bool
    {
        guard data.wrappedValue.containsIndex(indexPath.item) else { return false }
        switch selectionMode
        {
        case .none: return false
        case .highlighting:
...
@available(iOS 13.0, *)
internal extension ASSectionBindingDataSource
{
    mutating func setSelfSizingConfig(config: @escaping SelfSizingConfig)
    {
        selfSizingConfig = config
...
@available(iOS 13.0, *)
internal extension ASDKSectionDataSource
{
    mutating func setSelfSizingConfig(config: @escaping SelfSizingConfig)
    {
        selfSizingConfig = config
